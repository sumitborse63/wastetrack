...
interface TransferDao {
    ...
    @Query("SELECT * FROM transfers WHERE toRecyclerId = :recyclerId OR toRecyclerId = 'REC-AMBAD-01' OR toRecyclerId LIKE 'REC%' OR toRecyclerId LIKE '%recycler%' ORDER BY initiatedAt DESC")
    fun getByRecycler(recyclerId: String): Flow<List<TransferEntity>>

    @Query("SELECT SUM(COALESCE(weightAtDestination, weightAtSource)) FROM transfers WHERE (toRecyclerId = :recyclerId OR toRecyclerId = 'REC-AMBAD-01' OR toRecyclerId LIKE 'REC%' OR toRecyclerId LIKE '%recycler%') AND status = 'VERIFIED'")
    fun getRecycledWeightSum(recyclerId: String): Flow<Float?>

    @Query("SELECT COUNT(*) FROM transfers WHERE (toRecyclerId = :recyclerId OR toRecyclerId = 'REC-AMBAD-01' OR toRecyclerId LIKE 'REC%' OR toRecyclerId LIKE '%recycler%') AND status = :status")
